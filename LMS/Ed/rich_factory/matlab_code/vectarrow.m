...
%   $Revision: 1.0
  ...
  elseif max(size(p0))==2
      if max(size(p1))==2
          ...
          x1 = p1(1);
          y1 = p1(2);
          plot([x0;x1],[y0;y1],'LineWidth',4);   % Draw a line between p0 and p1
	  ...
	  dy=y1-y0;
dx=x1-x0;    
      hu = [x1-dy*beta; x1+beta*dx; x1+dy*beta];
      hv = [y1+dx*beta; y1+beta*dy; y1-dx*beta];

         
          hold on
	  fill(hu,hv,'b')
%          plot(hu(:),hv(:),'LineWidth',4)  % Plot arrow head
          grid on
          xlabel('x')
          ylabel('y')
          hold off
