...
%            (rhowC(iz)*wC(iz)-rhowC(iz-1)*wC(iz-1))/dz...
            cp*rhof(iz)*theta(iz)*(Exner(iz)-Exner(iz-1))/dz
          - rhof(iz)*g);
    endfor
    w = rhow./rhof;
    rhowC = interpToCell(rhow);
    ...
    for iz = 2:length(theta)-1
        rhoThetaiz = rhofOld(iz)*theta(iz) - dt/dz*...
            (rhowC(iz)*thetaC(iz) - rhowC(iz-1)*thetaC(iz-1));
        theta(iz) = rhoThetaiz/rhof(iz);
    endfor
    thetaC = interpToCell(theta);

%    % Lorenz update of potential temperature
%    for iz = 1:length(thetaC)
%        rhoThetaiz = rhoOld(iz)*thetaC(iz) - dt/dz*...
%                     (rhow(iz+1)*theta(iz+1) - rhow(iz)*theta(iz));
%        thetaC(iz) = rhoThetaiz/rho(iz);
%    endfor
%    theta = interpToFace(thetaC);
    theta(1) = thetaBottom;
    theta(end) = thetaTop;
